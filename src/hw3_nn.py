...
class GaussianNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2),
        ...
        x = numpy.linspace(-k,k, filter_shape[2])
        y = numpy.linspace(-k,k, filter_shape[3])
        X,Y=numpy.meshgrid(x,y)
        Gv=numpy.exp(-(X**2+Y**2)/(2*sigma**2))
        Gv=(Gv/numpy.sum(Gv.reshape(-1)))
        Gv = numpy.tile(Gv, (filter_shape[0],filter_shape[1],1,1) )
        Gv = Gv.astype(theano.config.floatX)
        G_kernel = theano.shared(Gv, borrow = True)
...
def train_nn(train_model, validate_model, test_model,
    ...
    print('Best validation score of %f %% obtained at iteration %i, '
          ...
          (best_validation_loss * 100., best_iter + 1, 100 - test_score * 100.))
